...
function cardHighlight() {
  $('#game_board').on('mouseover', function(e) {
    $(e.target)
      .closest($('.shade'))
      .css('border', '2px solid #0000ff');
  });

  $('#game_board').on('mouseout', function(e) {
    $(e.target)
      .closest($('.shade'))
      .css('border', '2px solid #619a61');
  });
}
...
function gameover(deckSize, pairs) {
  if (
    (deckSize === '52' && pairs === 26) ||
    (deckSize === '26' && pairs === 13)
  ) {
...
function checkPair() {
  ...
  if (
    first.attr('data-card') &&
    first.attr('data-card') === second.attr('data-card')
  ) {
    cardRemoval(first);
    cardRemoval(second);
    pairs++;
